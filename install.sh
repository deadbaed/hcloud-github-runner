...
# Register runner
/usr/local/bin/forgejo-runner register --no-interactive --instance ${GITHUB_SERVER_URL} --token ${MY_GITHUB_RUNNER_REGISTRATION_TOKEN} --name "${MY_NAME}" --labels "${MY_NAME},self-hosted:host://-self-hosted,hetzner" 
